...
export const convertFile = async (file, setConvertedFile) => {
  console.log("converting file");
  const response = await cloudConvert.jobs.create({
    tasks: {
      "import-task": {
  ...
  // const uploadTask = response.tasks.filter(task => task.name === 'import-task')[0];
  console.log("hi")
  const jobId = response.id;
  const convertedFileUrl = await cloudConvert.jobs.wait(jobId);
  console.log("here")
  setConvertedFile(convertedFileUrl.tasks[2].result.files[0].url)
};
